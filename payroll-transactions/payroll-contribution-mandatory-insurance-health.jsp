...
<title>HRMS | Health Insurance</title>
...
<body>
							...
							<div class="ibox-title custom-ibox-title">
	                           ...
	                           <div class="ibox-tools">	
	                               <button class="btn btn-primary btn-xs"
										title="Add an employee to health insurance"
										data-toggle="modal"
										data-target="#addInsuranceContributionModal">
								...
								<div class="row">
									<div class="col-md-12">
										<div class="table-responsive">
											<table
												class="table table-sm table-striped table-bordered table-hover dataTables-Insurance"
												id="dataTables-Insurance">
												<thead>
													<tr>
														<th>S/N</th>
														<th>Employee&nbsp;</th>
												...
												<tbody>
													<c:if test="${not empty insurances}">
														<c:forEach var="insurance" items="${insurances}" varStatus="status">
															<tr>
																<td width="30px">${status.count}</td>
																<td><c:out value="${insurance.fullName}" /></td>
																<td><c:out value="${insurance.insuranceprovider}" /></td>
																<td><c:out value="${insurance.insurancenumber}" /></td>
																<td>
																<fmt:parseDate value="${insurance.datestart}" var="datestartParsed" pattern="yyyy-MM-dd" />
	...
	<!-- Modals -->
	<div class="modal inmodal" id="addInsuranceContributionModal"
			tabindex="99999" role="dialog" aria-hidden="true">
		<!-- Add Modal -->
		<jsp:include page="payroll-contribution-mandatory-insurance-health-add.jsp" />
		<!-- End Add Modal-->
	</div>
	<!-- Mainly scripts -->
	...
	<script>
		...
		function GetInsuranceContributionServiceProviders(){
			$.ajax({
				type: 'GET',
				url: '${pageContext.request.contextPath }/v1/payroll-insurance-service-providers.json',
				success: function(providers) {				
					...
					$('#insuranceproviderid').trigger('change'); 
	    	    ...
	    	    error: function(jqXHR, textStatus, errorThrown) {
	    	        ...
	    	        $.alert({
	    	            title: 'Error!',
	    	            content: 'Failed to retrieve insurance service providers',
	    	        });
	    	    }
			}); 
		}
		...
		function GetInsuranceTypes(){
			$.ajax({
				type: 'GET',
				url: '${pageContext.request.contextPath }/v1/payroll-insurance-types.json',
				success: function(types) {				
					...
					$('#loaderInsuranceTypes').hide(SPIN_TOGGLE_TRANSLATION_DURATION); // hide loader
	...
	<script>
		...
		// ----------------------------------------------------------
		$(function(){
			$('#addInsuranceContributionModal').on('show.bs.modal', function () {
				  //alert("Show");
			});
			$('#addInsuranceContributionModal').on('shown.bs.modal', function () {
				//alert("Shown")
				// retrieve employees
				GetEmployees();
				// Get insurance types
				GetInsuranceTypes();
				// Get providers
				GetInsuranceContributionServiceProviders();
			});
		});
	</script>
	<script>
		...
		$(document).ready(function(){
			$('#dataTables-Insurance').DataTable({
				...
				buttons : [
					{extend : 'copy'},
					{extend : 'csv'},
					{extend : 'excel',title : 'Employees Insurance Fund'},
					{extend : 'pdf',title : 'Employees Insurance Fund'},

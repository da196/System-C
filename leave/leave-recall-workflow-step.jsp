...
<head>
   ...
   <title>HRMS | Leave Recall Workflow Step</title>
...
<div class="col-lg-12">
    <div class="tabs-container">
        <ul class="nav nav-tabs">
            <li class="active"><a data-toggle="tab" href="#tab-3"> <i class="fa fa-map-marker"></i> Leave Recall Workflow Step</a></li>
        </ul>
	...
	<div class="modal inmodal" id="addTrainingWorkflowStep"
		...
		<!-- Add Modal -->
		<jsp:include page="leave-recall-workflow-step-add.jsp" />
		<!-- End Add Modal-->
	</div>
	...
	<div class="modal inmodal" id="editTrainingWorkflowStep"
			tabindex="99999" role="dialog" aria-hidden="true">
		<!-- Add Modal -->
		<jsp:include page="leave-recall-workflow-step-edit.jsp" />
		<!-- End Add Modal-->
	</div>
...
<script src="<c:url value="/resources/dist/js/FormValidation.min.js" />"></script>
	...
	<script src="<c:url value="/resources/js/validation/ui_validation_leave_recall_workflow-step-add.js" />"></script>
	<script src="<c:url value="/resources/js/validation/ui_validation_leave_recall_workflow-step-edit.js" />"></script>
<!-- Page-Level Scripts -->
<script>
...
$(document).ready(function() {
    ...
    $(document).ready(function(){
    	...
    	$.ajax({
    		url: '${pageContext.request.contextPath}/get-leave-recall-workflow-name-listAjax',
    		type: 'GET',
    		success:function(data,textStatus,jqXHR){
    			if(!data){
    				alert("No Data Received");
    ...
    // show details
    	function showTraningDetails(id){ 	
    		...
    		if(id){			
    			...
    			var $apiUrl = "${pageContext.request.contextPath}/v1/leave-recall-workflow-step-byID/"+id;
    			$.ajax({
    				url:$apiUrl,
    				type:'GET',
    				statusCode: {            	    	
            	    	208: function(responseObject, textStatus, jqXHR) {
            	            showError("Duplicate Leave Recall Workflow Step details not allowed.");
            	        },
            	        401: function(responseObject, textStatus, jqXHR) {
            	        	showError("You are not authorized to peform this action.");
            	        },
            	        404: function(responseObject, textStatus, jqXHR) {
            	        	showError("You are Leave Recall Workflow Step Details not found");
            	        },
            	        ...
            	        503: function(responseObject, textStatus, errorThrown) {
            	        	showError("Leave Recall Workflow Step service unavailable");
            	        }           
            	    },
    				success: function(data, textStatus, jqXHR){
    					console.log(data);
    				...
    				error:function(jqXHR, textStatus, errorThrown){
    					console.log("Failed to retrieve Leave Recall Workflow Step details");
    				}
    			});
    		}
    	}
    	...
    	function deleteTraningDetails(id){
    		if(id){			
    			...
    			var $apiUrl = "${pageContext.request.contextPath}/v1/leave-recall-workflow-step-delete/"+id;
    			
                $.confirm({
                    title: 'Warning!',
                    content: 'Please confirm to delete Leave Recall Workflow Step.',
                    type: 'red',
                    buttons: {
                        ok: {
                            text: "ok",
                            ...
                            action: function(){
                    			$.ajax({
                    				...
                    				success: function(data, textStatus, jqXHR){
                                        ...
                                        $.confirm({
                                            title: 'Congrats!',
                                            content: 'Leave Recall Workflow Step successfully deleted.',
                                            type: 'green',
                                            buttons: {
                                                ok: {
                                                    text: "ok",

...
<head>
   ...
   <title>HRMS | Leave Workflow</title>
...
<div class="col-lg-12">
    <div class="tabs-container">
        <ul class="nav nav-tabs">
            <li class="active"><a data-toggle="tab" href="#tab-3"> <i class="fa fa-map-marker"></i> Leave Workflow</a></li>
        </ul>
                ...
                <div class="panel-body">
										<p align="right">
											<button type="button"
												...
												data-toggle="modal" data-target="#addTrainingWorkflow">+ Add
												Workflow</button>
										</p>
										<br>
										<div class="table-responsive">
											<table
												...
												<tbody>
													<c:if test="${not empty trainingWorkflow}">
														<c:forEach var="trainingWorkflow"
															items="${trainingWorkflow}" varStatus="status">
															<tr class="gradeX">
																<td width="30px">${status.count}</td>
																<td><c:out value="${trainingWorkflow.name}" /></td>
																<td><c:out value="${trainingWorkflow.code}" /></td>
																<td><c:out value="${trainingWorkflow.description}" /></td>
																<td><c:out
																		value="${trainingWorkflow.supervisordesignation}" /></td>
																<td>
																	<button onClick="showTraningDetails('${trainingWorkflow.id}');"
																	...
																	<button
																		onClick="deleteTraningDetails('${trainingWorkflow.id}');"
	...
	<div class="modal inmodal" id="addTrainingWorkflow"
		...
		<!-- Add Modal -->
		<jsp:include page="leave-workflow-add.jsp" />
		<!-- End Add Modal-->
	</div>
	...
	<div class="modal inmodal" id="editTrainingWorkflow"
			tabindex="99999" role="dialog" aria-hidden="true">
		<!-- Add Modal -->
		<jsp:include page="leave-workflow-edit.jsp" />
		<!-- End Add Modal-->
	</div>
...
src="<c:url value="/resources/dist/js/FormValidation.min.js" />"></script>
	...
	<script src="<c:url value="/resources/js/validation/ui_validation_leave_workflow-add.js" />"></script>
	<script src="<c:url value="/resources/js/validation/ui_validation_leave_workflow-edit.js" />"></script>
<!-- Page-Level Scripts -->
<script>
    ...
    $(document).ready(function(){
        $('.dataTables-leaveTypes').DataTable({
            ...
            buttons: [
                { extend: 'copy'},
                {extend: 'csv'},
                {extend: 'excel', title: 'Training Workflow'},
                {extend: 'pdf', title: 'Training Workflow'},
    ...
    // show details
    	function showTraningDetails(id){
    		...
    		if(id){			
    			...
    			var $apiUrl = "${pageContext.request.contextPath}/v1/leave-workflow-view-details/"+id;
    			$.ajax({
    				url:$apiUrl,
    				type:'GET',
    				statusCode: {            	    	
    				...
    				error:function(jqXHR, textStatus, errorThrown){
    					console.log("Failed to retrieve Leave Workflow details");
    				}
    			});
    		}
    	}
    	...
    	function deleteTraningDetails(id){
    		if(id){			
    			...
    			var $apiUrl = "${pageContext.request.contextPath}/v1/leave-workflow-delete/"+id;
    			
                $.confirm({
                    title: 'Warning!',
                    content: 'Please confirm to delete Leave Workflow.',
                    type: 'red',
                    buttons: {
                        ok: {
                            text: "ok",
                            ...
                            action: function(){
                    			$.ajax({
                    				...
                    				statusCode: {            	    	
                            	    	208: function(responseObject, textStatus, jqXHR) {
                            	            showError("Duplicate Leave Workflow details not allowed.");
                            	        },
                            	        401: function(responseObject, textStatus, jqXHR) {
                            	        	showError("You are not authorized to peform this action.");
                            	        },
                            	        404: function(responseObject, textStatus, jqXHR) {
                            	        	showError("You are Leave Workflow Details not found");
                            	        },
                            	        412: function(responseObject, textStatus, jqXHR) {
                            	        	showError("Your request is not valid, please reveiew and submit again");
                            	        },
                            	        ...
                            	        503: function(responseObject, textStatus, errorThrown) {
                            	        	showError("Leave Workflow service unavailable");
                            	        }           
                            	    },
                    				success: function(data, textStatus, jqXHR){
                    					console.log(JSON.stringify(data));
                                        $.confirm({
                                            title: 'Congrats!',
                                            content: 'Leave Workflow successfully deleted.',
                                            type: 'green',
                                            buttons: {
                                                ok: {
                                                    text: "ok",

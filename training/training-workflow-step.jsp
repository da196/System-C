...
<head>
   ...
   <title>HRMS | Training Workflow Step</title>
...
<div class="col-lg-12">
    <div class="tabs-container">
        <ul class="nav nav-tabs">
            <li class="active"><a data-toggle="tab" href="#tab-3"> <i class="fa fa-map-marker"></i> Training Workflow Step</a></li>
        </ul>
        <div class="tab-content">
            <div id="tab-3" class="tab-pane active">
                <div class="panel-body">
		           ...
		           <tbody>
		           <c:if test="${not empty trainingWorkflowStep}">
    				<c:forEach var="trainingWorkflowStep" items="${trainingWorkflowStep}" varStatus="status">
		            <tr class="gradeX">
		                <td width="30px">${status.count}</td>
		                <td><c:out value="${trainingWorkflowStep.workflow}"/></td>						
						<td><c:out value="${trainingWorkflowStep.approverdesignation}"/></td>
						<td><c:out value="${trainingWorkflowStep.approverdesignationnext}"/></td>
						<td><c:out value="${trainingWorkflowStep.approverdesignationprev}"/></td>
						<td><c:out value="${trainingWorkflowStep.stepnumber}"/></td>
						<td><c:out value="${trainingWorkflowStep.description}"/></td>
																<td>
																	<button onClick="showTraningDetails('${trainingWorkflowStep.id}');"
																	...
																	<button
																		onClick="deleteTraningDetails('${trainingWorkflowStep.id}');"
	...
	<div class="modal inmodal" id="addTrainingWorkflowStep"
		...
		<!-- Add Modal -->
		<jsp:include page="training-workflow-step-add.jsp" />
		<!-- End Add Modal-->
	</div>
	...
	<div class="modal inmodal" id="editTrainingWorkflowStep"
			tabindex="99999" role="dialog" aria-hidden="true">
		<!-- Add Modal -->
		<jsp:include page="training-workflow-step-edit.jsp" />
		<!-- End Add Modal-->
	</div>
...
<script src="<c:url value="/resources/dist/js/FormValidation.min.js" />"></script>
	...
	<script src="<c:url value="/resources/js/validation/ui_validation_training_workflow-step-add.js" />"></script>
	<script src="<c:url value="/resources/js/validation/ui_validation_training_workflow-step-edit.js" />"></script>
<!-- Page-Level Scripts -->
<script>
...
$(document).ready(function() {
    ...
    $(document).ready(function(){
    	...
    	$.ajax({
    		url: '${pageContext.request.contextPath}/get-training-workflow-name-listAjax',
    		type: 'GET',
    		success:function(data,textStatus,jqXHR){
    			if(!data){
    				alert("No Data Received");
    ...
    function showTraingDetailsModal(data){
    	...
    	$('#id').val(data.id);
    	$('#workflowidEdit').val(data.workflowid);
    	...
    	$.each(data,function(i,item){	
			...
			$("#approverdesignationprevidEdit").append('<option value="'+ data.approverdesignationprevid +'">'+ data.approverdesignationprev +'</option>');
			$("#approverdesignationidEdit").append('<option value="'+ data.approverdesignationid +'">'+ data.approverdesignation +'</option>');
			$("#approverdesignationnextidEdit").append('<option value="'+ data.approverdesignationnextid +'">'+ data.approverdesignationnext +'</option>');
		});	
    ...
    // show details
    	function showTraningDetails(id){ 	
    		...
    		if(id){			
    			...
    			var $apiUrl = "${pageContext.request.contextPath}/v1/training-workflow-step-byID/"+id;
    			$.ajax({
    				url:$apiUrl,
    				type:'GET',
    				statusCode: {            	    	
    	...
    	function deleteTraningDetails(id){
    		if(id){			
    			...
    			var $apiUrl = "${pageContext.request.contextPath}/v1/training-workflow-step-delete/"+id;
    			
                $.confirm({
                    title: 'Warning!',
                    content: 'Please confirm to delete Training Workflow.',
                    type: 'red',
                    buttons: {
                        ok: {
                            text: "ok",
                            ...
                            action: function(){
                    			$.ajax({
                    				...
                    				success: function(data, textStatus, jqXHR){
                                        ...
                                        $.confirm({
                                            title: 'Congrats!',
                                            content: 'Training Workflow successfully deleted.',
                                            type: 'green',
                                            buttons: {
                                                ok: {
                                                    text: "ok",

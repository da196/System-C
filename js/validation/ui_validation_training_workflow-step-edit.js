...
document.addEventListener('DOMContentLoaded', function(e) {
 ...
 const editTrainingWorkflowStepBtn = document.getElementById('editTrainingWorkflowStepBtn');
 const formUpdateTrainingWorkflowStep = document.getElementById('formUpdateTrainingWorkflowStep');
 const fv = FormValidation.formValidation(formUpdateTrainingWorkflowStep, {
   fields: { 
	   workflowid:{
     		validators: {
                 notEmpty: {
 ...
 editTrainingWorkflowStepBtn.addEventListener('click', function() {
     fv.validate().then(function(status) {
             if(status === 'Valid'){

            	  var $apiUrl = $('#formUpdateTrainingWorkflowStep').attr('action');
                  var $data = $('#formUpdateTrainingWorkflowStep').serializeObject();//.serializeArray();//serialize();
                 ...
                 statusCode: {
                     208: function(responseObject, textStatus, jqXHR) {
                         showError("Duplicate Training Workflow Step not allowed.");
                     },
                     ...
                     503: function(responseObject, textStatus, errorThrown) {
                             showError("Training Workflow Step service unavailable");
                     }
                 },
...
//                             $('#submitTrainingBtn').attr("disabled", true);
                             $.confirm({
                                 title: 'Congrats!',
                                 content: 'Training Workflow Step successfully Updated.',
                                 type: 'green',
                                 buttons: {
                                     ok: {
                                         text: "ok",

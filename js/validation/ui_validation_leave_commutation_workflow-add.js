...
document.addEventListener('DOMContentLoaded', function(e) {
 ...
 const addLeaveCommutationWorkflowBtn = document.getElementById('addLeaveCommutationWorkflowBtn');
 const formAddLeaveCommutationWorkflow = document.getElementById('formAddLeaveCommutationWorkflow');
 const fv = FormValidation.formValidation(formAddLeaveCommutationWorkflow, {
   fields: { 
     	name:{
     		validators: {
                 notEmpty: {
...
// });
 // click listener
 addLeaveCommutationWorkflowBtn.addEventListener('click', function() {
     fv.validate().then(function(status) {
             if(status === 'Valid'){

             var $apiUrl = $('#formAddLeaveCommutationWorkflow').attr('action');
             var $data = $('#formAddLeaveCommutationWorkflow').serializeObject();//.serializeArray();//serialize();
...
//                             $('#submitTrainingBtn').attr("disabled", true);
                             $.confirm({
                                 title: 'Congrats!',
                                 content: 'Leave Commutation Workflow successfully added.',
                                 type: 'green',
                                 buttons: {
                                     ok: {
                                         text: "ok",

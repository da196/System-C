...
document.addEventListener('DOMContentLoaded', function(e) {
 ...
 const editLeaveRecallWorkflowBtn = document.getElementById('editLeaveRecallWorkflowBtn');
 const formEditLeaveRecallWorkflow = document.getElementById('formEditLeaveRecallWorkflow');
 const fv = FormValidation.formValidation(formEditLeaveRecallWorkflow, {
   fields: { 
     	name:{
     		validators: {
                 notEmpty: {
...
// });
 // click listener
 editLeaveRecallWorkflowBtn.addEventListener('click', function() {
     fv.validate().then(function(status) {
             if(status === 'Valid'){

             var $apiUrl = $('#formEditLeaveRecallWorkflow').attr('action');
             var $data = $('#formEditLeaveRecallWorkflow').serializeObject();//.serializeArray();//serialize();
...
//                             $('#submitTrainingBtn').attr("disabled", true);
                             $.confirm({
                                 title: 'Congrats!',
                                 content: 'Leave Recall workflow successfully Updated.',
                                 type: 'green',
                                 buttons: {
                                     ok: {
                                         text: "ok",

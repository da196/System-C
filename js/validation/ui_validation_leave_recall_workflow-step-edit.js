...
document.addEventListener('DOMContentLoaded', function(e) {
 ...
 const editLeaveRecallWorkflowStepBtn = document.getElementById('editLeaveRecallWorkflowStepBtn');
 const formUpdateLeaveRecallWorkflowStep = document.getElementById('formUpdateLeaveRecallWorkflowStep');
 const fv = FormValidation.formValidation(formUpdateLeaveRecallWorkflowStep, {
   fields: { 
	   workflowid:{
     		validators: {
                 notEmpty: {
 ...
 editLeaveRecallWorkflowStepBtn.addEventListener('click', function() {
     fv.validate().then(function(status) {
             if(status === 'Valid'){

            	  var $apiUrl = $('#formUpdateLeaveRecallWorkflowStep').attr('action');
                  var $data = $('#formUpdateLeaveRecallWorkflowStep').serializeObject();//.serializeArray();//serialize();
                 ...
                 statusCode: {
                     208: function(responseObject, textStatus, jqXHR) {
                         showError("Duplicate Leave Recall Workflow Step not allowed.");
                     },
...
//                             $('#submitTrainingBtn').attr("disabled", true);
                             $.confirm({
                                 title: 'Congrats!',
                                 content: 'Leave Recall Workflow Step successfully Updated.',
                                 type: 'green',
                                 buttons: {
                                     ok: {
                                         text: "ok",

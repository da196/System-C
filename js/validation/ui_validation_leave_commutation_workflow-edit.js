...
document.addEventListener('DOMContentLoaded', function(e) {
 ...
 const editLeaveCommutationWorkflowBtn = document.getElementById('editLeaveCommutationWorkflowBtn');
 const formEditLeaveCommutationWorkflow = document.getElementById('formEditLeaveCommutationWorkflow');
 const fv = FormValidation.formValidation(formEditLeaveCommutationWorkflow, {
   fields: { 
     	name:{
     		validators: {
                 notEmpty: {
...
// });
 // click listener
 editLeaveCommutationWorkflowBtn.addEventListener('click', function() {
     fv.validate().then(function(status) {
             if(status === 'Valid'){

             var $apiUrl = $('#formEditLeaveCommutationWorkflow').attr('action');
             var $data = $('#formEditLeaveCommutationWorkflow').serializeObject();//.serializeArray();//serialize();
...
//                             $('#submitTrainingBtn').attr("disabled", true);
                             $.confirm({
                                 title: 'Congrats!',
                                 content: 'Leave Commutation workflow successfully Updated.',
                                 type: 'green',
                                 buttons: {
                                     ok: {
                                         text: "ok",

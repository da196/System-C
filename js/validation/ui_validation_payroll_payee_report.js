            	    ...
            	    success: function(data, textStatus, jqXHR) {
            	    	if(data){
                	        ...
                	        if(status==200 || status==208){
                	          ...
                	          s += '<th>Payee </th>';
                	          s += '<th>Job Title</th>';
                	          s += '<th>Employment Category</th>';
                			  s += '<th>National ID</th>';
                	          s += '</tr>';
                	          s += '</thead>';
                	          s += '<tbody >';
                				$.each(responseData.payelist, function(index,resData) {                   
                                	s += '<tr>';
                                	s += '<td>' + (index + 1) + '</td>';
                                	...
                                	var payepay = parseFloat(resData.payepay);
                    	        	var payepayFormatted = numeral(payepay).format('0,0.00');
                                	s += '<td>' + payepayFormatted + '</td>';

                                	s += '<td>' + resData.designation + '</td>';
                					
                					s += '<td>' + resData.employmentCategory + '</td>';
                    	      ...
                    	      $('#datatables').DataTable({
                      	            ...
                      	            buttons: [
                      	                {extend: 'copy', title: 'PAYEE Report of - ' + journalDate},
                      	                {extend: 'csv', title: 'PAYEE Report of - ' + journalDate},
                      	                {extend: 'excel', title: 'PAYEE Report of - ' + journalDate},
                      	                {extend: 'pdf', title: 'PAYEE Report of - ' + journalDate},
                      	                {extend: 'print',

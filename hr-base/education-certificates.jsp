...
<title>HRMS | Education Certificates</title>
												...
												<div class="panel-heading">
													<div class="panel-options">
														<ul class="nav nav-tabs">
															<li class="active"><a href="#tab-1" data-toggle="tab">Employee education certificates <span class="label label-info" id="requests"></span></a></li>
												...
												<div class="panel-body">
													<div class="tab-content">
														<div class="tab-pane active" id="tab-1">
															<div class="feed-activity-list">
																...
																<div class="table-responsive">
																	<table class="footable table table-stripped toggle-arrow-tiny table-hover employees-all-certificates" id="allEmployeesCertificates">
																		<thead>
																			<tr>
																				<th>ID</th>
														...
														<div class="tab-pane" id="tab-2">
															...
															<div class="table-responsive">
																<table
																	class="footable table table-stripped toggle-arrow-tiny table-hover pending-educations">
																	<thead>
																		<tr>
																			<th>ID</th>
																				<th data-hide="phone">Employee Name</th>
																	...
																	<tbody class="non-approved-educations">
...
function applyTable2Certificate(){
	if(applied==0){
		$('.employees-all-certificates').DataTable({
	        dom: '<"html5buttons"B>lTfgitp',
	        buttons: [
	            { extend: 'copy'},
	            {extend: 'csv'},
	            {extend: 'excel', title: 'All Employee Certificates'},
	            {extend: 'pdf', title: 'All Employee Certificates'},
...
function pendingEducations(){
	if(pendingEdu==0){
		$('.pending-educations').DataTable({
	        dom: '<"html5buttons"B>lTfgitp',
	        buttons: [
	            { extend: 'copy'},
	            {extend: 'csv'},
	            {extend: 'excel', title: 'Pending Educations for Approval'},
	            {extend: 'pdf', title: 'Pending Educations for Approval'},
...
$(document).ready(function(){	
	$.ajax({
		url: '${pageContext.request.contextPath}/get-education-cerificatesAjax',
		type: 'GET',
		success:function(data,textStatus,jqXHR){
			if(!data){
				...
				$.each(data,function(i,item){
					var approvalUrl = '${pageContext.request.contextPath}/approve-education.htm/'+item.employeeid+'/'+item.id;
					$(".all-educations").append('<tr>'
													+'<td>'+ ++i +'</td>'
													...
													+'<td><button class="btn btn-xs btn-success" onClick="educationDocumentViewer(\''+item.attachmentUri+'\','+item.approved+',\''+item.approvalComment+'\',\''+approvalUrl+'\','+item.employeeid+','+item.id+')"><i class="fa fa-file-pdf-o"></i> View</button></td>'
												 +'</tr>');	
				});	
				// apply style
	...
	$.ajax({
		url: '${pageContext.request.contextPath}/get-nonApproved-educationsAjax',
		type: 'GET',
		success:function(data,textStatus,jqXHR){
			if(!data){
				alert("No Data Received");
				...
				$.each(data,function(i,item){
					var approvalUrl = '${pageContext.request.contextPath}/approve-education.htm/'+item.employeeid+'/'+item.id;
					$(".non-approved-educations").append('<tr>'
													+'<td>'+ ++i +'</td>'
													+'<td>'+ item.employeename +'</td>'
													+'<td>'+ item.courseName +'</td>'
													+'<td>'+ item.startYear +'</td>'
													...
													+'<td>'+ item.instituteName +'</td>'
													+'<td>'+ item.countryname +'</td>'
													+'<td><button class="btn btn-xs btn-warning" onClick="educationDocumentViewer(\''+item.attachmentUri+'\','+item.approved+',\''+item.approvalComment+'\',\''+approvalUrl+'\','+item.employeeid+','+item.id+')"><i class="fa fa-file-pdf-o"></i> Approve</button></td>'
												 +'</tr>');					
				});	
				// apply style
				pendingEducations();

...
<title>HRMS | Short Courses</title>
												...
												<div class="panel-heading">
													<div class="panel-options">
														<ul class="nav nav-tabs">
															<li class="active"><a href="#tab-1" data-toggle="tab">All Short Courses <span class="label label-info" id="requests"></span></a></li>
												...
												<div class="panel-body">
													<div class="tab-content">
														<div class="tab-pane active" id="tab-1">
															<div class="feed-activity-list">
																...
																<div class="table-responsive">
																	<table class="footable table table-stripped toggle-arrow-tiny table-hover employees-all-short-courses" id="shortCourses">
																		<thead>
																			<tr>
																				<th>ID</th>
														...
														<div class="tab-pane" id="tab-2">
															...
															<div class="table-responsive">
																<table
																	class="footable table table-stripped toggle-arrow-tiny table-hover pending-short-courses">
																	<thead>
																		<tr>
																			<th>ID</th>
																				<th data-hide="phone">Employee Name</th>
																	...
																	<tbody class="non-approved-short-courses">
...
function applyTable2Certificate(){
	if(applied==0){
		$('.employees-all-short-courses').DataTable({
	        dom: '<"html5buttons"B>lTfgitp',
	        buttons: [
	            { extend: 'copy'},
	            {extend: 'csv'},
	            {extend: 'excel', title: 'All Short Courses'},
	            {extend: 'pdf', title: 'All Short Courses'},
...
var pendingShortCoz = 0;
function pendingEducations(){
	if(pendingShortCoz==0){
		$('.pending-short-courses').DataTable({
	        dom: '<"html5buttons"B>lTfgitp',
	        buttons: [
	            { extend: 'copy'},
	            {extend: 'csv'},
	            {extend: 'excel', title: 'Pending Short Courses for Approval'},
	            {extend: 'pdf', title: 'Pending Short Courses for Approval'},
...
$(document).ready(function(){	
	$.ajax({
		url: '${pageContext.request.contextPath}/get-short-coursesAjax',
		type: 'GET',
		success:function(data,textStatus,jqXHR){
			if(!data){
				...
				$.each(data,function(i,item){
					var approvalUrl = '${pageContext.request.contextPath}/approve-short-course.htm/'+item.employeeid+'/'+item.id;
					$(".all-short-courses").append('<tr>'
													+'<td>'+ ++i +'</td>'
													...
													+'<td><button class="btn btn-xs btn-success" onClick="shortCourseDocumentViewer(\''+item.uri+'\','+item.approved+',\''+item.approvalComment+'\',\''+approvalUrl+'\','+item.employeeid+','+item.id+')"><i class="fa fa-file-pdf-o"></i> View</button></td>'
												 +'</tr>');
	...
	$.ajax({
		url: '${pageContext.request.contextPath}/get-noApproved-short-coursesAjax',
		type: 'GET',
		success:function(data,textStatus,jqXHR){
			if(!data){
				alert("No Data Received");
				...
				$.each(data,function(i,item){
					var approvalUrl = '${pageContext.request.contextPath}/approve-short-course.htm/'+item.employeeid+'/'+item.id;
					$(".non-approved-short-courses").append('<tr>'
													+'<td>'+ ++i +'</td>'
													+'<td>'+ item.employeename +'</td>'
													+'<td>'+ item.coursename +'</td>'
													+'<td>'+ item.datestart +'</td>'
													...
													+'<td>'+ item.institution +'</td>'
													+'<td>'+ item.countryname +'</td>'
													+'<td><button class="btn btn-xs btn-warning" onClick="shortCourseDocumentViewer(\''+item.uri+'\','+item.approved+',\''+item.approvalComment+'\',\''+approvalUrl+'\','+item.employeeid+','+item.id+')"><i class="fa fa-file-pdf-o"></i> Approve</button></td>'
												 +'</tr>');
...
/******************* short courses viewer and reject functions **********************/
